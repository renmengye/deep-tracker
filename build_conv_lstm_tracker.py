...
def compute_soft_IOU_score(pred_heat_map, gt_heat_map):
    ...
    valid_len = tf.reduce_sum(tf.to_float(gt_area > 0))
    ...
    return tf.reduce_sum(intersect_area) / (tf.reduce_sum(union_area) + 1.0e-5)
...
def build_tracking_model(opt, device='/cpu:0'):
    ...
    with tf.device(get_device_fn(device)):
        ...
        conv_lstm_state[-1] = tf.concat(3, [tf.zeros(tf.pack(
            [batch_size, conv_lstm_h, conv_lstm_w, conv_lstm_hidden_depth * 2 - 1])), tf.expand_dims(init_heat_map, 3)])
